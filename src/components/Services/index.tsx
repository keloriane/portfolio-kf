...
const Services: React.FC<ServicesProps> = ({ satoshi, clash, services }) => {
  ...
  const titleRefs = services.map(() =>
    React.useRef<HTMLDivElement | null>(null),
  );
  const subtitleRefs = services.map(() =>
    React.useRef<HTMLDivElement | null>(null),
  );
  const numberRefs = services.map(() =>
    React.useRef<HTMLDivElement | null>(null),
  );
  const descriptionRefs = services.map(() =>
    React.useRef<HTMLDivElement | null>(null),
  );
  ...
  const containerBoxRefs = services.map(() =>
    React.useRef<HTMLDivElement | null>(null),
  );
  ...
  useLayoutEffect(() => {
    ...
    services.forEach((service, index) => {
      ...
      gsap.to(titleRefs[index]?.current, {
    ...
    <section className="py-[200px] ">
      <Container className="flex flex-col">
        <Col colStart={[2, 2, 2, 2]} colEnd={[26, 26, 15, 15]} className="">
          ...
          <h2 className={twMerge('font-bold uppercase', 'service-title')}>
            What do i do ?{' '}
          </h2>

          <h3 className="service-subtitle">
            As a freelancer i try to have a variety of tools in my disposal. But
            we need to stop somwhere in order to not get.{' '}
          </h3>
        </Col>
      </Container>
      <Container className="mt-[60px]">
        <Col colStart={[2, 2, 2, 11]} colEnd={[26, 26, 26, 26]}>
          {services.map((service, index) => (
            <div
              className=" flex flex-col py-[20px] border-b border-t-black border-b-black"
              key={index}
            ...
            >
              ...
              >
                ...
                <div className={twMerge('text-[45px] ', clash)}>
                  <AnimatedText
                    reactRef={titleRefs[index]}
                    text={service.title}
                    gap={'5px'}
                    className={twMerge(
                      'font-bold text-[20px] md:text-[38px] lg:text-[38px] xl:text-[48px]',
                      clash,
                    )}
                    duration={0.5}
                    splitBy="word"
                ...
                <div>
                  ...
                  >
                    ...
                    <circle
                      ...
                      stroke={hover[index] ? '#BE2525' : 'black'}
                      strokeWidth="0.755102"
                    />
                  </svg>
                </div>
              </div>
                    ...
                    <div className="flex items-center flex-col gap-[16px] md:gap-[35px] lg:gap-[50px] justify-start m-auto">
                      {
                        <AnimatedText
                          ...
                          as="p"
                          className={twMerge(
                            'capitalize lg:text-[22px] md:text-[16px] text-[12px] sm:gap-[2px] md:gap-[10px] xl:gap-[10px] justify-start',
                          )}
                        />
                      }
                      <div className="grid grid-cols-[2fr_1fr] gap-[20px] w-full ">
                        ...
                        >
                          <div
                            className="flex items-center h-full gap-[10px]"
                            ref={techs[index]}
                          >
                            {service.icons.map((image, i) => (
                              <div
                                className="w-[50%] h-[50%]  flex flex-wrap items-center"
                                key={i}
                              >
                                <Image
                                  src={image.url}
