      ...
      <Col colStart={[2 , 2,2,  11]} colEnd={[26, 26, 26, 26]} >
          {
            services.map((service , index) => (
              <div className="flex flex-col py-[20px] border-b border-t-black border-b-black" key={index} onClick={() => handleTab(index)} >
                <div className='flex items-center justify-between' ref={boxRefs[index]}>
                  ...
                  <div className={twMerge('text-[45px]' , clash)}>
                    <h3 className={twMerge("font-regular text-[20px] md:text-[48px] lg:text-[45px]" , clash)} >{service.title}</h3>
                  </div>
                  </div>
                  <div>
                    <svg className='lg:h-[74px] md:h-[50px] h-[36px]' width="74" height="74" viewBox="0 0 74 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <circle cx="37" cy="37" r="36.6224" transform="rotate(-90 37 37)" stroke="black" strokeWidth="0.755102"/>
                    </svg>
                  </div>  
                </div>
                <div>
                  <div  ref={containerBoxRefs[index]} className="sm:h-[200px] md:h-[250px] lg:h-[300px] xl:h-[400px]">
                {activeIndex === index ? ( 
                <div  className='flex items-center flex-col gap-[16px] md:gap-[35px] lg:gap-[50px] justify-start'>
                 {
