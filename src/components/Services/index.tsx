...
const services = [
  ...
  },
  {
    ...
    description: "Je suis un développeur spécialisé en React et Typescript. Passionné par le code, j'ai également un penchant pour le design, fusionnant esthétique et fonctionnalité dans chaque projet sur lequel je travaille.",
    images: [nextjsimage, wordpressimage, reactimage, vuejsimage, angularjsimage],
    subtitle: ["Progressive Web Apps", "Single Page Applications", "Web Applications"],
    caption: backend
...
const Services = ({ satoshi, clash }: { satoshi: string, clash: string }) => {
  ...
  useLayoutEffect(() => {
    ...
    const timelines = services.map((service, index) => {
      ...
      gsap.set(numberRefs[index].current, { color: "black" })
      gsap.set(titleRefs[index].current, { color: "black", y: 0 })
      gsap.set([subtitleRefs[index].current?.childNodes], { y: 20, color: "black", opacity: 0 })
      ...
      .fromTo(
        ...
        .fromTo(
          ...
          { color: "black", duration: 0.2, },
          { color: "white", },-0.12
          )
          .fromTo(titleRefs[index].current, { duration: 0.2, color: "black" }, { opacity: 1, color: "white", }, 0.2)
          .fromTo([subtitleRefs[index].current?.childNodes], { opacity:0, duration: 0.05, y:20, color: "black" }, { opacity: 1, stagger: 0.1, y:0,  color: "white", },-.01)
  ...
  return (
    <section>
      <Container className='w-screen mt-[300px]'>
        <Col colStart={1} colEnd={5}>
        <Title title="Services" />
        </Col>
        <Col colStart={2} colEnd={25}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 h-[800px]">
            {services.map((service, index) => (
              <div key={index} className="border-r-2 border-black p-5 relative overflow-hidden" ref={containerBoxRefs[index]}>
                <div className="flex flex-col items-evenly  justify-between h-full">
                  <div className="flex flex-col text-black p-5">
                    <div className="relative z-10 flex items-center">
                      ...
                      <h3
                        className={twMerge("text-sNumber font-bold ml-5", clash)} ref={numberRefs[index]}>
                        {`0${index + 1}`}
                      </h3>
                    </div>
                    <div>
                      <h3 className={twMerge("text-[3rem] ml-4", satoshi)} ref={titleRefs[index]}>{parse(service.title)}</h3>
                    </div>
                  </div>
                    <span className={twMerge("overflow-hidden text-[1.77rem] uppercase p-5", satoshi)} ref={subtitleRefs[index]}>
                      {
                ...
                <div ref={boxRefs[index]} className="absolute bottom-0 left-0  bg-black h-full w-full z-[-20]"></div>
              </div>
            ))}
          </div>
        </Col>
